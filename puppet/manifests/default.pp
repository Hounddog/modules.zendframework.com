...
define mysql_db (
  ...
  file { "/var/www/config/autoload/database.local.php":
    replace => "no", # this is the important property
    ensure  => "present",
    content => template('/var/www/config/autoload/database.local.php.dist'), 
    mode    => 644,
  }
}
...
if $github_values == undef {
  $github_values = hiera('github', false)
}

$client_id = $github_values['client_id']
$secret = $github_values['secret']

file { "/var/www/config/autoload/github.local.php":
    replace => "no", # this is the important property
    ensure  => "present",
    content => template('/var/www/config/autoload/github.local.php.dist'), 
    mode    => 644,
  }